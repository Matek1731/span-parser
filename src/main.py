...
if __name__ == '__main__':
    ...
    if args.vocab is not None:
        ...
        fm = features.FeatureMapper.load_json(args.vocab)
    elif args.train is not None:
        import features
        fm = features.FeatureMapper(args.train)
    ...
    if args.test is not None:
        from src import phrase_tree
        from src import network
        from src import parser

        test_trees = phrase_tree.PhraseTree.load_treefile(args.test)
        print('Loaded test trees from {}'.format(args.test))
        network = network.Network.load(args.model)
        print('Loaded model fromf: {}'.format(args.model))
        accuracy = parser.Parser.evaluate_corpus(test_trees, fm, network)
